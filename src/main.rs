...
pub fn confine_player_movement(
    ...
    if let Ok(mut player_transform) = player_query.get_single_mut() {
        ...
        let half_player_size: f32 = PLAYER_SIZE / 2.0; // 32
        let x_min = 0.0 - window.width() / 2.0 + half_player_size;
        let x_max = window.width() - half_player_size - window.width() / 2.0;
        let y_min = 0.0 - window.height() / 2.0 + half_player_size;
        let y_max = window.height() - half_player_size - window.height() / 2.0;
...
pub fn enemy_hit_player(
    ...
    if let Ok((player_entity, player_transform)) = player_query.get_single_mut() {
        for enemy_transform in enemy_query.iter() {
            ...
            let player_rad = PLAYER_SIZE / 2.0;
            let enemy_rad = PLAYER_SIZE / 2.0;

            if dist < player_rad + enemy_rad {
                println!("Game Over!");
                let sfx = asset_server.load("audio/explosion.ogg");
                audio.play(sfx);
...
pub fn player_hit_star(
    ...
    if let Ok((_player_entity, player_transform)) = player_query.get_single() {
        for (star_entity, star_transform) in star_query.iter() {
            let dist = player_transform.translation.distance(star_transform.translation);
            let player_rad = PLAYER_SIZE / 2.0;
            let star_rad = STAR_SIZE / 2.0;

            if dist < player_rad + star_rad {
                println!("Player hit star!");
                score.value += 1;
                let sfx = asset_server.load("audio/pickupCoin.ogg");
                audio.play(sfx);
...
pub fn update_score(score: Res<Score>) {
    if score.is_changed() {
        println!("Score: {0}", score.value.to_string());
    }
}
